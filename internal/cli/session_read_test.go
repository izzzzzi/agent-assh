...
func TestSessionReadConnectionClosedReturnsSessionUnreachable(t *testing.T) {
	...
	got := executeSessionJSONError(t, []string{"session", "read", "-s", "abcdef12", "--seq", "1", "--limit", "50"})

	if got["ok"] != false || got["error"] != "session_unreachable" {
		t.Fatalf("unexpected response: %#v", got)
	}
	hint, _ := got["hint"].(string)
	for _, want := range []string{"assh connect", "-i KEY", "-E PASSWORD_ENV",
		"--ssh-config ALIAS", "80.74.27.102", "root"} {
...
func TestSessionReadRawNotFoundReturnsJSONError(t *testing.T) {
	...
	hint, _ := got["hint"].(string)
	for _, want := range []string{"do not keep retrying this SID", "assh connect",
		"-i KEY", "-E PASSWORD_ENV", "--ssh-config ALIAS"} {
		if !strings.Contains(hint, want) {
			t.Fatalf("hint %q missing %q", hint, want)
		}
	}
