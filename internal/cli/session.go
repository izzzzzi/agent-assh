...
// registeredSessionLifecycleError maps SSH-level failures of a registered
// session (auth, connection, tmux missing, output gone) to
// session_unreachable/session_stale errors with a reconnect hint, so agents
// stop retrying dead SIDs and re-bootstrap with explicit auth instead.
func registeredSessionLifecycleError(cmd *cobra.Command, entry session.RegistryEntry,
	...
	case "tmux_missing":
		return writeError(cmd, "session_stale",
			"registered session host is reachable but tmux is unavailable: "+message, hint)
	default:
...
func reconnectHint(entry session.RegistryEntry) string {
	...
	return "do not keep retrying this SID; reconnect with explicit auth: " +
		"assh connect -H " + host + " -u " + user + " -i " + key + " -n " + name + " or " +
		"assh connect -H " + host + " -u " + user + " -E PASSWORD_ENV -n " + name + "; " +
		"prefer assh connect --ssh-config ALIAS -n " + name + " for repeat use"
}
