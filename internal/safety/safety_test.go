...
// dangerousCommandCases is the table of (command, expected rule) pairs that the
// safety classifier must flag. Kept at package level so the test runner stays
// under the funlen limit and the cases read as a flat list.
var dangerousCommandCases = []struct {
	name    string
	command string
	rule    string
}{
	...
	{name: "env assignment rm recursive", command: "VAR=x rm -rf /tmp/build", rule: "rm_recursive"},
	{name: "quoted env assignment rm recursive", command: "VAR='x y' rm -rf /tmp/build", rule: "rm_recursive"},
	{name: "env rm recursive", command: "env rm -rf /tmp/build", rule: "rm_recursive"},
	{name: "env split string rm recursive", command: "env -S 'rm -rf /tmp/build'", rule: "rm_recursive"},
	{name: "env long split string find delete", command: "env --split-string='find /tmp -delete'", rule: "find_delete"},
	...
	{
		name:    "command env split string find delete",
		command: "command env --split-string='find /tmp -delete'",
		rule:    "find_delete",
	},
	...
	{name: "bash shopt option operand rm recursive",
		command: "bash -O extglob -c 'rm -rf /tmp/build'", rule: "rm_recursive"},
	{name: "bash rcfile operand rm recursive",
		command: "bash --rcfile /dev/null -c 'rm -rf /tmp/build'", rule: "rm_recursive"},
	{name: "bash init file operand rm recursive",
		command: "bash --init-file /dev/null -c 'rm -rf /tmp/build'", rule: "rm_recursive"},
	...
	{name: "brace group rm recursive", command: "{ rm -rf /tmp/build; }", rule: "rm_recursive"},
	{name: "subshell group rm recursive", command: "(rm -rf /tmp/build)", rule: "rm_recursive"},
...
func TestCheckCommandBlocksDangerousCommands(t *testing.T) {
	for _, test := range dangerousCommandCases {
