...
func TestRunValidatesRequiredFields(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name: "host",
			req: Request{
				User: "root", Port: 22, Identity: "/tmp/id",
				TTL: time.Hour, Timeout: time.Second,
				HostKeyPolicy: "accept-new", StateDir: t.TempDir(),
			},
			want: "invalid_args",
		},
		{
			name: "port low",
			req: Request{
				Host: "example.com", User: "root", Port: 0, Identity: "/tmp/id",
				TTL: time.Hour, Timeout: time.Second,
				HostKeyPolicy: "accept-new", StateDir: t.TempDir(),
			},
			want: "invalid_args",
		...
		},
		{
			...
			req: Request{
				Host: "example.com", User: "root", Port: 22, Identity: "/tmp/id",
				TTL: time.Hour, Timeout: time.Second,
				HostKeyPolicy: "bad", StateDir: t.TempDir(),
			},
			want: "invalid_args",
		},
	}
